...
class SolutionsRemoverVisitor(FileVisitor):
    ...
    def visit_file(self, file: Path) -> None:
        text_with_reduced_content = remove_lines_within_limiters_from_string(
            file.read_text()
        )
        if is_cpp(file):
            text_with_reduced_content = remove_clang_tidy_comment_lines(
                text_with_reduced_content
            )
...
def remove_lines_within_limiters_from_string(multiline_string: str) -> str:
    ...
    return re.sub(
        minimal_length_between_markers_pattern, "", multiline_string, flags=re.DOTALL
    )
