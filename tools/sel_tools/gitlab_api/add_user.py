...
def find_gitlab_users_of_students(
    gitlab_instance: gitlab.Gitlab, students: list[Student]
) -> list[Student]:
    """Return list of students with their Gitlab users."""
    for student in students:
        try:
            student.gitlab_user = gitlab_instance.users.list(search=student.mail_addr)[
                0
            ]
        except IndexError:
